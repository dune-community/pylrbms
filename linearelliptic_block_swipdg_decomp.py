...
def discretize():
    ...
    lambda_bar = make_constant_function_1x1(grid, 1.0, name='lambda')
    ...
    for ii in range(grid.num_subdomains):
        ...
        for jj in neighborhood:
            for kk in neighborhood:
                nc_ops[jj, kk] = NonconformatyOperator(ii, jj, kk, block_op.source, grid, block_space,
                                                       global_rt_space, local_boundary_info,
        ...
        def assemble_estimator_residual_functional(lambda_xi):
            ...
            for jj in neighborhood:
                r1_ops[0, jj] = ResidualPartFunctional(f, ii, jj, jj, block_op.source, grid, block_space,
                                                       global_rt_space, local_boundary_info,
                                                       lambda_bar, lambda_xi, None, kappa)
        ...
        r2_ops, df_ops = zip(*(assemble_estimator_residual_diffusive_flux(lambda_xi, lambda_xi_prime)
                               for lambda_xi in diffusion_functions
                               for lambda_xi_prime in diffusion_functions))
        r1_ops = [assemble_estimator_residual_functional(lambda_xi) for lambda_xi in diffusion_functions]

        coefficients2 = [ProductParameterFunctional([c1, c2]) for c1 in coefficients for c2 in coefficients]

        operators['nc_{}'.format(ii)] = nc_op
        operators['r1_{}'.format(ii)] = LincombOperator(r1_ops, coefficients, name='residual_functional_{}'.format(ii))
    ...
    min_diffusion_evs = np.array([min_diffusion_eigenvalue(grid, ii, lambda_bar, kappa) for ii in
